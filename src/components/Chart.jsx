...
function Chart({ porCategoria }) {
  const dados = Object.entries(porCategoria).map(function ([nome, valor]) {
    return { nome, valor: parseFloat(valor.toFixed(2)) };
  });

  const total = dados.reduce(function (acc, d) {
    return acc + d.valor;
  }, 0);
  ...
  return (
    ...
    >
      <div className="chart__wrap">
        ...
        <ul
          className="chart__legenda"
          aria-label="Legenda do gráfico de categorias"
        >
          {dados.map(function (entry, index) {
